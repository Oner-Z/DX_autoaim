...
public:
		...
		: Node("armor_node")
	{
		...
		if (!use_comm_shm_) {
			...
			tx_pub_ = this->create_publisher<std_msgs::msg::String>(tx_topic, rclcpp::SystemDefaultsQoS());
		} else {
			setupCommShm();
			if (rx_sem_) {
				running_.store(true);
...
private:
	...
	void processFrame(cv::Mat &frame) {
		...
		try {
			if (gp.attack_mode == ARMOR) {
				...
				// 能量机关
				if (!wmi_) wmi_ = std::make_unique<WMIdentify>(gp);
				if (!wmp_) wmp_ = std::make_unique<WMPredict>(gp);
	...
	// 共享内存——串口
	struct ShmTx { uint32_t len; uint8_t data[64]; };
	struct ShmRx { uint32_t len; uint8_t data[512]; };
	...
	void setupCommShm() {
		...
		tx_shm_ = static_cast<ShmTx*>(mmap(nullptr, sizeof(ShmTx), PROT_READ | PROT_WRITE, MAP_SHARED, tx_fd_, 0));
		rx_shm_ = static_cast<ShmRx*>(mmap(nullptr, sizeof(ShmRx), PROT_READ | PROT_WRITE, MAP_SHARED, rx_fd_, 0));
		...
		if (tx_sem_ == SEM_FAILED || rx_sem_ == SEM_FAILED) {
			...
			if (tx_sem_ != SEM_FAILED) { sem_close(tx_sem_); tx_sem_ = nullptr; }
			if (rx_sem_ != SEM_FAILED) { sem_close(rx_sem_); rx_sem_ = nullptr; }
		}
	}

	void closeCommShm() {
		if (tx_shm_ && tx_shm_ != MAP_FAILED) { munmap(tx_shm_, sizeof(ShmTx)); tx_shm_ = nullptr; }
		if (rx_shm_ && rx_shm_ != MAP_FAILED) { munmap(rx_shm_, sizeof(ShmRx)); rx_shm_ = nullptr; }
	...
	void commRxLoop() {
		while (running_.load()) {
			if (!rx_sem_ || !rx_shm_) { std::this_thread::sleep_for(std::chrono::milliseconds(5)); continue; }
			// 等待数据到达
	...
	void setupImageShm() {
		...
		image_data_ = reinterpret_cast<uint8_t*>(reinterpret_cast<char*>(image_map_) + sizeof(ImageShmHeader));
	...
	void closeImageShm() {
		if (image_map_) { munmap(image_map_, image_shm_size_); image_map_ = nullptr; }
		if (image_fd_ >= 0) { close(image_fd_); image_fd_ = -1; }
	...
	void imageRxLoop() {
		while (running_.load()) {
			if (!image_sem_ || !image_hdr_) { std::this_thread::sleep_for(std::chrono::milliseconds(5)); continue; }
			...
			if (w == 0 || h == 0 || ch == 0 || step == 0 || len == 0) continue;
			// 只支持 BGR8
			if (ch != 3) continue;
